...
| [`fw/`](fw/) | Firmware — Rust + [embassy](https://embassy.dev) on STM32F072CB: key matrix, twin WS2812 chains, USB HID + vendor interface, DFU reboot, and an opt-in **Codex Micro compat mode** (the pad boots with the Codex Micro's USB identity and speaks ChatGPT Desktop's device protocol so the desktop app drives it natively — confirmed with Codex desktop 26.825 on macOS). See [`fw/README.md`](fw/README.md) |
