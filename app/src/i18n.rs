...
pub const STRINGS: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("settings_section_profile", "PROFILE", "配置", "設定檔", "プロファイル"),
    ("settings_section_app", "APP", "应用", "應用程式", "アプリ"),
    ("settings_section_pad", "PAD", "键盘", "鍵盤", "パッド"),
    ("settings_section_agents", "CODING AGENTS", "编程智能体", "程式設計代理", "コーディングエージェント"),
    ("settings_section_data", "DATA & PERMISSIONS", "数据与权限", "資料與權限", "データと権限"),
    ("settings_section_community", "COMMUNITY", "社区", "社群", "コミュニティ"),
    ("open_firmware_sheet", "Open", "打开", "開啟", "開く"),
    ("no_pad_connected", "No pad connected", "未连接键盘", "未連接鍵盤", "パッドが接続されていません"),
    ...
    ("windows_dfu_driver", "WINDOWS DFU DRIVER", "WINDOWS DFU 驱动", "WINDOWS DFU 驅動程式", "WINDOWS DFU ドライバー"),
    ("dfu_driver_required", "WinUSB is not bound to the STM32 bootloader. Set up the driver, then retry Install.", "STM32 引导程序尚未绑定 WinUSB。请设置驱动后重试安装。", "STM32 開機載入程式尚未繫結 WinUSB。請設定驅動程式後重試安裝。", "STM32 ブートローダーに WinUSB が割り当てられていません。ドライバー設定後に再度インストールしてください。"),
    ("dfu_driver_one_time", "Only the one-time migration from firmware 0.9.0 or older (and a bootloader reinstall) goes through ROM DFU and needs a WinUSB binding for STM32 BOOTLOADER (0483:df11). Regular updates use the pad's own bootloader and need no driver.", "仅从 0.9.0 或更早固件的一次性迁移（以及重新安装引导程序）需要经由 ROM DFU，并为 STM32 BOOTLOADER（0483:df11）绑定 WinUSB。常规更新通过键盘自带的引导程序进行，无需驱动。", "僅從 0.9.0 或更早韌體的一次性遷移（以及重新安裝開機載入程式）需要經由 ROM DFU，並為 STM32 BOOTLOADER（0483:df11）繫結 WinUSB。一般更新透過鍵盤內建的開機載入程式進行，無需驅動程式。", "ROM DFU を使い STM32 BOOTLOADER（0483:df11）への WinUSB 設定が必要なのは、0.9.0 以前のファームウェアからの一度限りの移行（およびブートローダーの再インストール）だけです。通常の更新はパッド内蔵のブートローダーで行われ、ドライバーは不要です。"),
    ("open_dfu_driver_setup", "DFU driver setup", "DFU 驱动设置", "DFU 驅動程式設定", "DFU ドライバー設定"),
    ("dfu_driver_steps", "In Zadig: Options > List All Devices; choose STM32 BOOTLOADER; select WinUSB; Install/Replace Driver. Then return and click Install again.", "在 Zadig 中：选项 > 列出所有设备；选择 STM32 BOOTLOADER；选择 WinUSB；安装/替换驱动。然后返回并再次点击安装。", "在 Zadig 中：選項 > 列出所有裝置；選擇 STM32 BOOTLOADER；選擇 WinUSB；安裝/取代驅動程式。然後返回並再次點擊安裝。", "Zadig で Options > List All Devices、STM32 BOOTLOADER、WinUSB、Install/Replace Driver の順に選び、戻って再度インストールしてください。"),
    ...
    ("keep_the_pad_powered_if_the", "Keep the pad powered during an update. If a bootloader update is interrupted, the pad simply stays in bootloader mode and Install resumes it. Only the one-time ROM DFU migration cannot resume: unplugging during it may require SWD recovery.", "更新期间请保持键盘供电。若引导程序更新被中断，键盘会停留在引导程序模式，再次「安装」即可续传。只有一次性的 ROM DFU 迁移无法续传：中途拔出可能需要 SWD 恢复。", "更新期間請保持鍵盤供電。若開機載入程式更新被中斷，鍵盤會停留在開機載入程式模式，再次「安裝」即可續傳。只有一次性的 ROM DFU 遷移無法續傳：中途拔出可能需要 SWD 復原。", "更新中はパッドの電源を切らないでください。ブートローダー経由の更新が中断されてもパッドはブートローダーモードのままで、「インストール」で再開できます。再開できないのは一度限りの ROM DFU 移行だけで、その途中で抜くと SWD による復旧が必要になる場合があります。"),
    ...
    ("app_update_ready", "Verified package ready", "已验证的安装包已就绪", "已驗證的安裝套件已就緒", "検証済みパッケージの準備完了"),
    ("app_update_open_dmg", "Open DMG", "打开 DMG", "開啟 DMG", "DMG を開く"),
    ("app_update_download_package", "Download package", "下载安装包", "下載安裝套件", "パッケージをダウンロード"),
    ("app_update_downloading_package", "Downloading package", "正在下载安装包", "正在下載安裝套件", "パッケージをダウンロード中"),
    ("app_update_open_package", "Open package", "打开安装包", "開啟安裝套件", "パッケージを開く"),
    ...
    ("reboot_into_dfu", "Reboot into ROM DFU", "重启进入 ROM DFU", "重新啟動進入 ROM DFU", "ROM DFU モードで再起動"),
    // ---- resident bootloader (firmware 0.10.0+) ----
    ("recovery_badge", "RECOVERY", "恢复模式", "復原模式", "リカバリー"),
    ("recovery_mode_detected", "Pad in recovery mode detected", "检测到键盘处于恢复模式", "偵測到鍵盤處於復原模式", "リカバリーモードのパッドを検出しました"),
    ("update_firmware", "Update firmware", "更新固件", "更新韌體", "ファームウェアを更新"),
    ("bootloader_label", "BOOTLOADER", "引导程序", "開機載入程式", "ブートローダー"),
    ("bootloader_app_valid", "Firmware {v} · valid", "固件 {v} · 有效", "韌體 {v} · 有效", "ファームウェア {v} · 有効"),
    ("bootloader_app_unstamped", "Firmware {v} · unstamped development build", "固件 {v} · 未签章的开发版本", "韌體 {v} · 未簽章的開發版本", "ファームウェア {v} · スタンプなしの開発ビルド"),
    ("bootloader_app_none", "No valid firmware installed", "未安装有效固件", "未安裝有效韌體", "有効なファームウェアがありません"),
    ("bootloader_unopenable", "The bootloader is connected but could not be opened. On Linux, add the udev rule for 1209:0002.", "已检测到引导程序但无法打开。在 Linux 上请为 1209:0002 添加 udev 规则。", "已偵測到開機載入程式但無法開啟。在 Linux 上請為 1209:0002 新增 udev 規則。", "ブートローダーは接続されていますが開けませんでした。Linux では 1209:0002 の udev ルールを追加してください。"),
    ("bootloader_info_failed", "The bootloader is connected but did not answer BOOT_INFO: {e}", "已检测到引导程序，但未应答 BOOT_INFO：{e}", "已偵測到開機載入程式，但未回應 BOOT_INFO：{e}", "ブートローダーは接続されていますが BOOT_INFO に応答しませんでした: {e}"),
    ("bootloader_state_valid", "Valid", "有效", "有效", "有効"),
    ("bootloader_state_unstamped", "Unstamped", "未签章", "未簽章", "スタンプなし"),
    ("bootloader_state_none", "No valid firmware", "无有效固件", "無有效韌體", "有効なファームウェアなし"),
    ("install_firmware", "Install firmware", "安装固件", "安裝韌體", "ファームウェアをインストール"),
    ("boot_firmware", "Boot firmware", "启动固件", "啟動韌體", "ファームウェアを起動"),
    ("reboot_into_bootloader", "Reboot into bootloader", "重启进入引导程序", "重新啟動進入開機載入程式", "ブートローダーで再起動"),
    ("drops_the_pad_into_its_bootloader", "Restarts the pad in its update mode (1209:0002) and leaves it there — the same as holding the encoder switch while plugging in. No driver needed.", "让键盘重启进入更新模式（1209:0002）并停留在该模式——与插入时按住旋钮开关效果相同。无需驱动。", "讓鍵盤重新啟動進入更新模式（1209:0002）並停留在該模式——與插入時按住旋鈕開關效果相同。無需驅動程式。", "パッドを更新モード（1209:0002）で再起動し、そのまま待機させます。接続時にエンコーダースイッチを押し続けるのと同じです。ドライバーは不要です。"),
    ("reinstall_bootloader_via_rom_dfu", "Reinstall bootloader via ROM DFU", "通过 ROM DFU 重新安装引导程序", "透過 ROM DFU 重新安裝開機載入程式", "ROM DFU でブートローダーを再インストール"),
    ("rom_dfu_warning", "Only for reinstalling the bootloader itself, with a combined openmicro-fw image. This uses the chip's ROM DFU (0483:df11): Windows needs the WinUSB driver, and if this flash is interrupted the pad can only be recovered over SWD.", "仅用于使用组合版 openmicro-fw 镜像重新安装引导程序本身。此操作使用芯片的 ROM DFU（0483:df11）：Windows 需要 WinUSB 驱动，且写入中途中断的话只能通过 SWD 恢复键盘。", "僅用於使用組合版 openmicro-fw 映像重新安裝開機載入程式本身。此操作使用晶片的 ROM DFU（0483:df11）：Windows 需要 WinUSB 驅動程式，且寫入中途中斷的話只能透過 SWD 復原鍵盤。", "結合版 openmicro-fw イメージでブートローダー自体を再インストールする場合のみ使います。チップの ROM DFU（0483:df11）を使うため、Windows では WinUSB ドライバーが必要で、書き込みが中断されるとパッドは SWD でしか復旧できません。"),
    ...
    ("codex_compat_mode", "Codex Micro compat mode", "Codex Micro 兼容模式", "Codex Micro 相容模式", "Codex Micro 互換モード"),
    ("codex_compat_note", "Restarts the pad with the Codex Micro's USB identity so ChatGPT Desktop can drive it directly — agent lights, dial and stick included. This app and firmware updates keep working; the pad's own key bindings are bypassed meanwhile. Same as holding KEY 04 (second key of the second row) while plugging in; KEY 03 switches back.", "以 Codex Micro 的 USB 身份重启键盘，让 ChatGPT 桌面版可以直接驱动它——包括智能体指示灯、旋钮和摇杆。本应用和固件更新仍可正常使用；期间键盘自身的按键绑定会被绕过。等同于插入时按住 KEY 04（第二行第 2 个键）；按住 KEY 03 切回。", "以 Codex Micro 的 USB 身分重新啟動鍵盤，讓 ChatGPT 桌面版可以直接驅動它——包含代理指示燈、旋鈕與搖桿。本應用程式與韌體更新仍可正常使用；期間鍵盤自身的按鍵綁定會被略過。等同於插入時按住 KEY 04（第二列第 2 個鍵）；按住 KEY 03 切回。", "Codex Micro の USB 識別情報でパッドを再起動し、ChatGPT デスクトップから直接操作できるようにします（エージェントライト・ダイヤル・スティック対応）。このアプリとファームウェア更新はそのまま使えますが、その間パッド自身のキー割り当ては無効になります。接続時に KEY 04（2 段目の左から 2 番目）を押し続けるのと同じで、KEY 03 で元に戻ります。"),
    ("codex_compat_restarting", "Switching — the pad is saving the mode and restarting…", "正在切换——键盘正在保存模式并重启……", "正在切換——鍵盤正在儲存模式並重新啟動……", "切り替え中 — パッドがモードを保存して再起動しています…"),
    ("codex_compat_unsupported", "Needs a connected pad on firmware 0.8.0 or newer.", "需要连接固件 0.8.0 或更新版本的键盘。", "需要連接韌體 0.8.0 或更新版本的鍵盤。", "ファームウェア 0.8.0 以降のパッドを接続してください。"),
    ...
    ("beh_windows", "Windows", "Windows", "Windows", "Windows"),
    ("beh_system", "System", "系统", "系統", "システム"),
