...
pub const STRINGS: &[(&str, &str, &str, &str, &str)] = &[
    ...
    ("dfu_driver_one_time", "Firmware updates require a one-time WinUSB binding for STM32 BOOTLOADER (0483:df11).", "固件更新需要为 STM32 BOOTLOADER（0483:df11）一次性绑定 WinUSB。", "韌體更新需要為 STM32 BOOTLOADER（0483:df11）一次性繫結 WinUSB。", "ファームウェア更新には STM32 BOOTLOADER（0483:df11）への一度限りの WinUSB 設定が必要です。"),
