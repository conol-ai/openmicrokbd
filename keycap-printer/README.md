...
Skeleton mode skips the fill stage entirely: every pass traces only the edge contours, which engraves the icon as an outline. Shapes too thin to reach the 50% coverage threshold produce no contours and cannot start in this mode.

The Text tab engraves arbitrary text instead of an icon. The string is rendered with Chromium's font engine onto an offscreen canvas at 40 px/mm — one canvas pixel per supersample — and the alpha channel becomes the ink coverage function feeding the same raster pipeline, so text gets identical edge tracing, grayscale fills, scan-direction cycling, and skeleton support. Font family (enumerated from installed system fonts via `queryLocalFonts`, with a fallback list), weight, and size are selectable; size sets the em height in mm. Mirror, rotation, and fine offsets apply to text like they do to icons.
